 ...
 * by Spring bean manager only. For more detail, see {@link ScoutBootBeanProducer}
 *
 * @see ScoutBootBeanProducer
 */
public class ScoutBootBeanManager extends BeanManagerImplementor {

  private final ApplicationContext springApplicationContext;
  private final BeanDefinitionRegistry springBeanRegistry;

  public ScoutBootBeanManager(final ApplicationContext springApplicationContext, final BeanDefinitionRegistry springBeanRegistry) {
  ...
  protected <T> BeanImplementor<T> createBeanImplementor(final BeanMetaData beanData) {
    return new BeanImplementor<>(beanData, new ScoutBootBeanProducer<>(springApplicationContext));
  }
